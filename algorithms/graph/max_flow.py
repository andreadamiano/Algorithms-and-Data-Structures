...
def bfs(adj_dict: dict[dict[int]], node: int, sink_node: int):
        ...
        while queue:
            ...
            if curr_node == sink_node:
                return True, current_path, min_capacity, visited

            for neighbor, capacity in adj_dict[curr_node].items():
                if  capacity and neighbor not in visited:
                    queue.append((current_path + (neighbor, ), min(min_capacity, adj_dict[curr_node][neighbor])))

        return False, current_path, min_capacity, visited


def min_cut(adj_dict: dict[dict[int]], source_node: int, sink_node: int):
    """
    ...
    result, path, min_capacity, _ = bfs(residual_adj_dict, source_node, sink_node)
    while result :
        ...
        result, path, min_capacity, visited = bfs(residual_adj_dict, source_node, sink_node)

    # find the min cut by exploring all reacheable nodes
    edges = set()
    for source in visited:
        for sink in adj_dict[source].keys():
            if sink not in visited and adj_dict[source][sink]:
    ...
    while not stop:
        ...
        for vertex in range(1, len(vertices)):
            ...
            #inflow > outflow
                ...
                _, path, _, _= bfs(adj_dict, vertex, sink_node) #find the path using the original adjency dict 

            #outflow > inflow
            else:
                _, path, _, _ = bfs(adj_dict, source_node, vertex)
...
def max_flow(n_vertices, n_edges, source_node, sink_node, edges):
    ...
    for source, sink, indicator in edges:
        ...
        residual_adj_dict[sink][source] = 0 #initialize the reverse edge for the Ford-Fulkerson algorithm
    ...
    # print(adj_dict_flows)
    # print(vertices)

    saturated_edges = min_cut(residual_adj_dict, source_index, sink_index)
